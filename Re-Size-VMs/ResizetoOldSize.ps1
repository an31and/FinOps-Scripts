...
foreach ($server in $servers) {
    ...
    $oldSize = $server.NewSize
    $newSize = $server.OldSize
    ...
    try {
        ...
        if ($vmRunning -and $skuMatched) {
            ...
            Write-Log "VM '$vmName' failed to validate running state or size after resize" "ERROR"
            $status = "ValidationFailed"
        }

            Write-Log "Successfully resized $vmName from $oldSize to $newSize"
            $status = "Success"
        }
    } catch {
